...
def main() -> None:
    ...
    class ProjectionAuditController(QWidget):
        def __init__(self, viewer: napari.Viewer) -> None:
            ...
            next_unreviewed = QPushButton("Next unreviewed")
        ...
        def refresh_status(self) -> None:
            ...
            self.status_label.setText(
                f"Reviewed: {len(reviewed)}/{len(cases)} | Auditor: {auditor}"
            )
        ...
        def next_unreviewed(self) -> None:
            ...
            for offset in range(1, len(cases) + 1):
                ...
                if cases[position].candidate_code not in reviewed:
                    self.index = position
                    self.load_current()
                    return
            self.refresh_status()
            QMessageBox.information(
                ...
                "Every case in this role has a recorded projection decision.",
            )
