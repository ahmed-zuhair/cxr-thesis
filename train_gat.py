...
train_gat.py
============
Training loop for the GAT model. Reuses nih_dataset infrastructure.
Backbone is frozen, so only the graph head trains (~2M params).
Expected: ~10-12 min per epoch on Kaggle T4.

Usage:
    python train_gat.py \\
        --data_root /kaggle/input/datasets/organizations/nih-chest-xrays/data \\
        --backbone_ckpt /kaggle/working/densenet_best.pth \\
        --epochs 5 \\
        --batch_size 32 \\
        --lr 5e-4 \\
...
from gat_model import GATModel, count_trainable


def upload_to_hf(local_path: str, repo_id: str, token: str, path_in_repo: str):
    try:
...
def parse_args():
    ...
    p.add_argument("--backbone_ckpt", type=str, required=True,
                   help="Path to trained DenseNet checkpoint (best.pth)")
    ...
    p.add_argument("--no_knn", action="store_true",
                   help="Use only spatial edges, disable feature kNN")
    p.add_argument("--dropout", type=float, default=0.3)
    p.add_argument("--checkpoint_dir", type=str, default="/kaggle/working/checkpoints_gat")
...
def main():
    ...
    print(f"[model] loading GAT with frozen DenseNet from {args.backbone_ckpt}")
    model = GATModel(
    ...
    print(f"[model] use_knn={not args.no_knn}, knn_k={args.knn_k}")
    ...
    start_epoch = 0; best_mean_auc = 0.0
    if args.resume and os.path.exists(args.resume):
        ...
        print(f"[resume] from epoch {start_epoch}, best AUC: {best_mean_auc:.4f}")
